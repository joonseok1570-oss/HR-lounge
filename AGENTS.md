...
- 회사 도메인 접근 제어는 `login.html`, `api/auth.js`, `middleware.ts`를 기본 구조로 유지한다. Vercel 배포본은 사용자가 입력한 이메일이 `@solmedix.com` 도메인인지 서버에서 확인하고, 통과 시 `hr_lounge_access` HttpOnly 쿠키를 발급한다. 정적 화면만 숨기는 방식으로 되돌리지 않는다.
