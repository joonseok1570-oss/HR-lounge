...
function renderCategoryBoardCard(post, variant = "") {
  const image = getPostThumbnailImage(post);
  const group = getPostGroupDisplay(post);
  const subcategory = getPostSubcategory(post);
...
function renderCategoryTopicSections(categorySlug, posts) {
  const accentColors = ["#d86657", "#247f82", "#657dff", "#6f9272", "#c98621"];
  const category = getCategoryBySlug(categorySlug);
  const sections = getCategoryTaxonomy(category.slug)
    .map((group, index) => {
  ...
  if (!sections.length) {
    return `<p class="post-empty">아직 업로드된 ${escapeHtml(category.label)} 글이 없습니다. 글 작성을 눌러 첫 게시글을 올려보세요.</p>`;
  }
...
function renderCategorySectionBoard(categorySlug, posts) {
  return `
...
function renderPostList() {
  ...
  if (isSearchMode) {
    ...
    if (contentSectionDivider) {
      const category = getCategoryBySlug(currentBlogCategory);
      contentSectionDivider.hidden = isCategorySectionBoard;
  ...
  const canSortPosts =
    !isCategorySectionBoard && !isSearchMode && !activeBlogListQuery.trim() && activeBlogSort === "basic" && isEditorUnlocked && posts.length > 1;
  postList.classList.toggle("is-sortable", canSortPosts);
  postList.classList.toggle("is-culture-board", isCategorySectionBoard);

  postList.innerHTML = isCategorySectionBoard
    ? renderCategorySectionBoard(currentBlogCategory, posts)
    : posts.length
    ? posts
        .map(
          (post) => {
