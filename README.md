...
- `index.html`: 홈페이지 마크업
- `login.html`, `logout.html`: 회사/승인 계정 로그인과 HR Lounge 세션 로그아웃 화면
...
Vercel/Cloudflare 배포본은 `middleware.ts`, `api/auth.js`, `functions/`를 통해 회사 Google Workspace 계정, 승인된 Google 개인계정, 승인된 Naver 메일만 접속하도록 보호합니다. 로그인 화면은 `login.html`이며, Chrome에 허용된 계정이 연결되어 있으면 Google One Tap 또는 계정 선택으로 바로 입장할 수 있습니다. 이미 로그인된 사용자는 상단의 `로그아웃` 버튼 또는 `logout.html`에서 HR Lounge 세션을 종료할 수 있습니다.
...
- `HR_LOUNGE_ALLOWED_GOOGLE_EMAILS`: 선택 사항. 승인된 개인 Google 계정을 쉼표나 줄바꿈으로 입력합니다.
- `HR_LOUNGE_ALLOWED_NAVER_EMAILS`: 선택 사항. 승인된 Naver 메일을 쉼표나 줄바꿈으로 입력합니다. 현재 로그인은 Google OAuth 기반이므로 해당 Naver 메일은 Google 계정으로 선택할 수 있어야 합니다.
- `HR_LOUNGE_ALLOWED_EMAILS`: 선택 사항. 기존 호환용 일반 승인 메일 목록입니다. 새로 추가할 때는 위의 Google/Naver 전용 값을 권장합니다.
