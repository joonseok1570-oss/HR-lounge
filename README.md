...
Vercel 배포본은 `middleware.ts`와 `api/auth.js`를 통해 회사 이메일 도메인만 접속하도록 보호합니다. 로그인 화면은 `login.html`이며, 사용자가 `@solmedix.com` 이메일을 입력하면 서버가 도메인을 확인한 뒤 세션 쿠키를 발급합니다.

아래 환경변수만 Vercel Project Settings > Environment Variables에 있으면 동작합니다.

- `HR_LOUNGE_ALLOWED_DOMAIN`: 생략 가능. 기본값은 `solmedix.com`
- `HR_LOUNGE_SESSION_SECRET`: 로그인 쿠키 서명용 긴 임의 문자열

이 방식은 별도 계정 연동 없이 간단히 쓰는 도메인 게이트입니다. 실제 이메일 소유권 인증은 하지 않으므로, 보안 운영 시에는 저장소도 private 상태로 유지하는 것이 원칙입니다.
