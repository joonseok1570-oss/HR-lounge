...
Vercel 배포본은 `middleware.ts`와 `api/auth.js`를 통해 회사 Google Workspace 계정만 접속하도록 보호합니다. 로그인 화면은 `login.html`이며, Chrome에 회사 Google 계정이 연결되어 있으면 Google One Tap 또는 계정 선택으로 바로 입장할 수 있습니다.
